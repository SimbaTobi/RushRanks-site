...
(function(){
  ...
  for (const id of buttons){
    ...
    if (url && url.length > 6){ a.href = url; a.target = "_blank"; a.querySelector('.label').textContent = isIOS ? 'Get it on the App Store (iOS)' : 'Get it on the Play Store (Android)'; }
    else { a.href = 'mailto:support@rushranks.com?subject=Notify me when the app is live'; a.querySelector('.label').textContent = isIOS ? 'Download on iOS' : 'Get it on the Play Store (Android)'; }
  }
